...
-- 注意：由于users表没有role字段，这里使用简化的策略
-- 如果需要管理员权限，可以通过其他方式实现（如环境变量检查）
...
-- 7. 示例数据（可选）
-- 插入一些示例邮箱别名
-- INSERT INTO user_email_aliases (user_id, alias_email, notes) VALUES
--   ('user-uuid-here', 'alternative@example.com', '用户提供的备用邮箱');
